...
#                                             # another catalog template (EBR2-120)
#
# Env overrides: ACCEPTANCE_IMAGE, NRP_HAPROXY_CONTAINER, RESULTS_DIR,
#                NRP_BASE_URL, NRP_MQTT_HOST, NRP_MQTT_PORT, NRP_FS_USER,
#                NRP_FS_PASSWORD, START_TIMEOUT, NRP_TEMPLATE, NRP_CONFIG,
#                NRP_EXPECTED_PREFIX (and the deprecated HUSKY_TEMPLATE /
#                HUSKY_CONFIG aliases) — defaults in conftest.py.
set -euo pipefail
...
exec docker run --rm \
  ...
  -e NRP_FS_USER -e NRP_FS_PASSWORD -e NRP_MQTT_PORT \
  "$IMAGE" \
